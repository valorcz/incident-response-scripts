...
START_MARKER=re.escape(r'#@~^')
END_MARKER=re.escape(r'^#~@')

# Read the lines and convert it to string
script="".join(sys.stdin.readlines())

# We need to skip not only the start marker, but the trailing checksum as well.
# The checksum ends with '=='
s_marker = re.search("(?<="+START_MARKER+"[A-Za-z0-9+\/]{6}==).*", script)
if (s_marker is None):
    print("Start marker not found");
    exit();
# Take only the rest of the string -- that's where the encoded part resides.
script = s_marker.group(0)

# Ending part of the marker is preceded by '==' as well, we need to skip it.
e_marker = re.search(".*(?=[A-Za-z0-9+\/]{6}=="+END_MARKER+")", script)
if (e_marker is None):
    print("End marker not found");
    exit();
# Take only preceeding part of the script. That's where the encoded part resides.
script = e_marker.group(0)

# And now something completely different -- the decoding routine!
...
for char in (script):
   if special:
      special = 0
      char = char.translate(string.maketrans("&#!*$", "\n\r<>@"))
   elif (char < "\x80"): # encrypted?
     if itab[pos] == 0:
        char = dectab0[ord(char)]
     elif itab[pos] == 1:
        char = dectab1[ord(char)]
     elif itab[pos] == 2:
        char = dectab2[ord(char)]
     if (char == "\xFF"):
        special = 1

   if special == 0:
      output += char # add the final translation to the output
      pos = (pos + 1) % 64 # get to the next point in the translation table
